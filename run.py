...
def load_base_model_and_tokenizer(name):
    if args.openai_model is None:
        ...
        base_model = transformers.AutoModelForCausalLM.from_pretrained(name, **base_model_kwargs, force_download=True)
    else:
        base_model = None
    ...
    base_tokenizer = transformers.AutoTokenizer.from_pretrained(name, **optional_tok_kwargs, force_download=True)
...
def eval_supervised(data, model):
    ...
    detector = transformers.AutoModelForSequenceClassification.from_pretrained(model, force_download=True).to(DEVICE)
...
if __name__ == '__main__':
    ...
    print("Cache directory set to:", cache_dir)


    try:
        GPT2_TOKENIZER = transformers.GPT2Tokenizer.from_pretrained('gpt2', force_download=True)
    except Exception as e:
        print("Failed to load GPT-2 tokenizer:", e)
    ...
    if not args.baselines_only and not args.random_fills:
        ...
        mask_model = transformers.AutoModelForSeq2SeqLM.from_pretrained(mask_filling_model_name, **int8_kwargs, **half_kwargs, force_download=True)
        try:
    ...
    mask_tokenizer = transformers.AutoTokenizer.from_pretrained(mask_filling_model_name, model_max_length=n_positions, force_download=True)
